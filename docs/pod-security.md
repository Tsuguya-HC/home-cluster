...
| kanidm | restricted | |
| kyverno | restricted | |
| llm-gateway | restricted | Agent Router のデータプレーン（Envoy）。claude-code / claude-code-build の handler はここ経由。直行は taskflow-openrouter-smoke（意図的）と argo の pluto-check のみ |
